"""
Layout generator based on: https://github.com/NREL/WPGNN/blob/main/playgen.py
from the [1] paper.
...
layout_limits_run1 = {
    ...
    "num_turbines_lower_bound": 40,
    "num_turbines_upper_bound": 150,
    "domain_size_lower_bound": 40,
    "domain_size_upper_bound": 150,
}
...
def setup_eval_layout(boundary_dict):
    separation_low_bc = boundary_dict["separation_lower_bound"]
    separation_up_bc = boundary_dict["separation_upper_bound"]
    num_turbines_low_bc = boundary_dict["num_turbines_lower_bound"]
    num_turbines_up_bc = boundary_dict["num_turbines_upper_bound"]
    domain_size_low_bc = boundary_dict["domain_size_lower_bound"]
    domain_size_up_bc = boundary_dict["domain_size_upper_bound"]

    def eval_layout(layout, separation):
        domain_x = layout[:, 0].max() - layout[:, 0].min()
        domain_y = layout[:, 1].max() - layout[:, 1].min()
        domain_max = max(domain_x, domain_y)
        domain_min = min(domain_x, domain_y)

        passed_state = True
        if separation.min() < separation_low_bc or separation.max() > separation_up_bc:
            logger.debug("Seperation not in range")
            passed_state = False
        elif len(layout) < num_turbines_low_bc or len(layout) > num_turbines_up_bc:
            logger.debug("Number of turbines not in range")
            passed_state = False
        elif domain_min < domain_size_low_bc or domain_max > domain_size_up_bc:
            logger.debug("Domain size not in range")
            passed_state = False
...
class PLayGen:
    ...
    def set_spacing(self, spacing, spacing_units=None):
        ...
        if spacing_units is not None:
            assert spacing_units in [
                "m",
                "D",
            ], "Bad spacing units. Must be either 'm' or 'D'."
            self.spacing_units = spacing_units

        if spacing is not None:
            assert spacing > 0.0, "Bad inter-turbine spacing value."
        self.spacing = spacing

    def set_angle(self, angle, angle_units=None):
        # Set the general wind plant orientation.
        if angle_units is not None:
            assert angle_units in [
                "degrees",
                "radians",
            ], "Bad spacing units. Must be either 'degrees' or 'radians'."
            self.angle_units = angle_units
    ...
    def _interturbine_spacing_(self, x, y):
        ...
        D = (
            (np.expand_dims(x, axis=0) - np.expand_dims(x, axis=1)) ** 2
            + (np.expand_dims(y, axis=0) - np.expand_dims(y, axis=1)) ** 2
            + np.diag(np.nan * np.ones((x.size,)))
        )
    ...
    def _random_breaks_(self, N_turbs, N_breaks=None):
        ...
        if N_breaks is None:
            N_breaks = (
                np.random.randint(int(np.sqrt(N_turbs))) if N_turbs >= 5 else 0
            )  # WAS N_turbs/5
    ...
    def _distribute_turbines_(self, N_turbs, N_string):
        ...
        N_turbs_per_string = 2 * np.ones((N_string,), dtype=np.int32)
        count = 0
    ...
    def _generate_string_(self, N_turbs, R, theta, breaks, noise):
        ...
        for break_i in breaks:
            break_idx = np.insert(
                break_idx,
                np.random.randint(break_idx.size),
                np.zeros((break_i,), dtype=bool),
            )
    ...
    def random_single_string_layout(self):
        ...
        N = (
            int(round(self._truncated_lognormal_(25, 100, 50, 20)[0]))
            if self.N_turbs is None
            else self.N_turbs
        )
    ...
    def random_multiple_string_layout(self):
        ...
        N = (
            int(round(self._truncated_lognormal_(25, 200, 125, 50)[0]))
            if self.N_turbs is None
            else self.N_turbs
        )
        ...
        for i, N_i in enumerate(N_per_string):
            ...
            d_turb_all = self._interturbine_spacing_(
                np.concatenate((x, x_i_c)), np.concatenate((y, y_i_c))
            )
    ...
    def random_parallel_string_layout(self):
        ...
        N_string = int(np.random.normal(loc=np.sqrt(N) / 2.0, scale=1))
        N_string = 2 if N_string < 2 else N_string
        ...
        offset_step = np.random.uniform(low=1.0, high=3.5)
        ...
        for i, (N_i, breaks_i) in enumerate(zip(N_per_string, string_breaks)):
            ...
            # Generate string of turbines
            x_i, y_i = self._generate_string_(N_i, R_i, 0.0, breaks_i, noise_level)

            # Compute horizontal/vertical offset of each parallel string
            x_i += np.random.uniform(low=R_i / 2 - R, high=R - R_i / 2)
            if i > 0:
                y_i -= np.max(y_i) + d_turb * (
                    offset_step + abs(np.random.normal(loc=0, scale=0.5))
                )
        ...
        wind_plant = (
            phi
            @ np.concatenate(
                (x.reshape((1, N)) - np.mean(x), y.reshape((1, N))) - np.mean(y), axis=0
            )
        ).T
